...
const userUpdateService = async (id: any, data: any, email_auth: any): Promise<any> => {
  validateData(data, ['name', 'email', 'is_adm', 'is_active', 'is_faq', 'is_post', 'is_product', 'is_user'])

  const userRepo = AppDataSource.getRepository(Users);
  const {name, email, is_adm, is_active, is_faq, is_post, is_product, is_user} = data;
